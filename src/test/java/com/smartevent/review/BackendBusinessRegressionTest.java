...
/** Regression coverage for the backend business review of 2026-09-10. */
class BackendBusinessRegressionTest {
    @Test
    ...
    void splittingItemsMustNotBypassFourTicketsPerOrder() {
        ...
        phase.setId(phaseId);
        when(events.findByIdForShare(eventId)).thenReturn(Optional.of(event));
    ...
    void signedReturnWithWrongAmountMustNotCompleteOrder() {
        ...
        params.put("vnp_TmnCode", "REVIEW"); params.put("vnp_TransactionStatus", "00");
        when(properties.getTmnCode()).thenReturn("REVIEW");
        ...
        var handler = new VNPayCallbackHandler(payments, mock(PaymentWebhookEventRepository.class), lifecycle,
                properties, new ObjectMapper(), completion, mock(com.smartevent.modules.payment.service.PaymentReconciliationService.class));
